...
# 20230608 Max Liu for anawering in Chinese
...
def handle_message(q):
    ...
    sources_str = ''
    # 20230608 Max Liu for answer in chinese
    lang = language.detect_content_language(answer)
    answer = language.translate(answer,lang,'zh-Hant')
    send('Answer:'+ answer)
    if len(sources) > 0:
        for s in sources: 
            try:
